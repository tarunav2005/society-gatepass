...
const navItems = [
  { to: "/resident", icon: Bell, label: "Requests", end: true },
  { to: "/resident/invite", icon: UserPlus, label: "Invite Guest" },
  { to: "/resident/staff", icon: UserCog, label: "My Staff" },
  { to: "/resident/history", icon: History, label: "History" },
  { to: "/resident/profile", icon: Settings, label: "Profile" },
];
      ...
      <aside className="w-64 shrink-0 glass !rounded-none border-r border-slate-200 dark:border-slate-800 flex flex-col">
        <div className="p-6 border-b border-slate-200 dark:border-slate-800">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            {user?.tower?.name
              ? `${user.tower.name} - ${user.flat?.flatNumber}`
              : "Resident Panel"}
          </p>
        </div>
        <nav className="flex-1 p-4 space-y-1">
          {navItems.map(({ to, icon: Icon, label, end }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition relative ${isActive ? "text-primary-600 dark:text-primary-400" : "text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800/50"}`
              }
            >
              {({ isActive }) => (
                <>
